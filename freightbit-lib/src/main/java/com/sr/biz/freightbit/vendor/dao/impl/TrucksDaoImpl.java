...
public class TrucksDaoImpl extends HibernateDaoSupport implements TrucksDao {
    ...
    public List <Trucks> findTrucksByTruckCode(String truckCode) {
        ...
        try {
            Query query = getSessionFactory().getCurrentSession().createQuery("from Trucks t where t.truckCode = :truckCode");
            query.setParameter("truckCode", truckCode);
            List<Trucks> results = (List<Trucks>) query.list();
            log.debug("Find by truckCode successful, result size: " + results.size());
            return results;
        } catch (RuntimeException e) {
    ...
    public List<Trucks> findOrderItemsByPlateNumber(String plateNumber) {
        log.debug("Finding Trucks instance");
        try {
            Query query = getSessionFactory().getCurrentSession().createQuery("from Trucks t where t.plateNumber = :plateNumber");
            query.setParameter("plateNumber", plateNumber);
            List<Trucks> results = (List<Trucks>) query.list();
            log.debug("Find by plateNumber successful, result size: " + results.size());
            return results;
        } catch (RuntimeException e) {
            log.error("Find by plateNumber failed", e);
            throw e;
        }    }
    ...
    public Trucks findTruckByCode(String truckCode) {
        ...
        try {
            Query query = getSessionFactory().getCurrentSession().createQuery("from Trucks t where t.truckCode = :truckCode");
            query.setParameter("truckCode", truckCode);
            Trucks results = (Trucks) query.list();
            return results;
        } catch (RuntimeException e) {
            log.error("Find by truckCode failed", e);
            throw e;
